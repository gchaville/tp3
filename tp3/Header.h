...
public:
    ...
    void Inserer(noeud *&fils, int d); // insère le noeud de valeur d à l’arbre.
    void Supprimer(noeud *&fils, int d); // Supprime le nœud de valeur d de l’arbre.
	noeud* SupprimerMin(noeud *&fils);
    void Afficher_Arbre(noeud *&fils); /*Affiche les éléments de l’arbre niveau par niveau. Un bonus de
    ...
    int Afficher_hauteur(noeud *&fils); //Affiche la hauteur de l’arbre.
    void Afficher_Ascendant(noeud *&fils, int d); // Affiche les ascendants du noeud de valeur d.
    void Archiver (noeud *&fils);// Archiver en implémentation séquentielle l’arbre dont la racine est racine
